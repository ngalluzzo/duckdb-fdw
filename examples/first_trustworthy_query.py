...
def main() -> int:
    ...
    parser.add_argument(
        "--json",
        action="store_true",
        help="emit the verified result as one JSON object",
    )
    args = parser.parse_args()

    artifact = pathlib.Path(args.artifact).resolve(strict=True)
    sql_path = pathlib.Path(__file__).with_name("first-trustworthy-query.sql")
    ...
    try:
        ...
        require_equal("query rows", rows, EXPECTED_ROWS)
    finally:
        connection.close()
