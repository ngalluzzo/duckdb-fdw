...
pins = json.loads((root / "release/0.4.0/pins.json").read_text())
scripts = "\n".join(
    path.read_text()
    for path in (
...
if "release/0.4.0/pins.json" not in scripts:
    raise SystemExit("native developer workflow does not select current 0.4 pins")
