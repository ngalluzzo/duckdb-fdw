...
def verify_authority(repository: pathlib.Path, rfc: str, authority: dict[str, object]) -> dict[str, object]:
    ...
    expected_verifier_digest = authority.get("verifier_sha256")
    if expected_verifier_digest is None and isinstance(verifier_record, dict):
        expected_verifier_digest = verifier_record.get("digest")
    if expected_verifier_digest is not None:
        if not isinstance(expected_verifier_digest, str) or not re.fullmatch(
            r"sha256\.[0-9a-f]{64}", expected_verifier_digest
        ):
            raise AssertionError(f"RFC {rfc} evidence verifier digest authority is malformed")
        ...
        if observed != expected_verifier_digest:
