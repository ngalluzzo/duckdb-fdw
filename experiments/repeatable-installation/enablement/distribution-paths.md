...
Extensions as the target direction; compatibility, lifecycle, and support
...
cannot answer the remaining reserved product questions: which DuckDB/platform
cells are promised; who operates updates, rollbacks, retention, incident
response, and support; or how historical versions behave. Those must remain
explicit RFC decisions rather than being inferred from a successful unsigned
local install. MIT also does not replace a dependency-license audit.
