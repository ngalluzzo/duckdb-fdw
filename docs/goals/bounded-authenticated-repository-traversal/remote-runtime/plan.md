...
| New `src/include/duckdb_api/internal/link_pagination.hpp` and `src/link_pagination.cpp` | Own the independently testable combined Link field-value parser, strict next-target validation, typed positive page extraction, exact-increment and seen-page checks, and canonical next-request patch. Consume a Runtime execution policy derived from public `PaginationPlan` accessors; never import Connector declarations or accept caller URL/header authority. | Pure tests map normalized metadata and current typed state to either one canonical next page, exhaustion, or a stable redacted policy error. No DuckDB, curl, secret, or native catalog constructor is needed. |
...
The split is responsibility-driven. Link grammar and typed transitions change
with protocol policy; resource accounting changes with operational envelopes;
transport changes with dependency header delivery; decode changes with JSON
source shape; and the executor composes those services. They must not be folded
into one larger `http_scan_executor.cpp` state machine or a catch-all utility.
...
parameters; malformed quoting, escaping, delimiters, target syntax, or
parameters fail closed. It requires zero or one link whose relation set
contains the exact accepted `next` relation and rejects ambiguous multiple-next
targets.
...
| --- | --- | --- | --- |
| Link grammar and typed transition | New `link_pagination.*` and `link_pagination_tests.cpp` | RFC 0007 fixes the accepted grammar and authority; production plan adapter waits for the Semantics accessor shape | Does not edit executor, transport, Connector, planner, adapter, or root build files |
...
2. **Provider-interface gate.** Connector and Semantics land the closed
   pagination/resource provider shapes and focused fixtures without public
   relation activation. Runtime confirms the immutable plan contains every
   execution fact and requires no Connector import or query-field inference.
3. **Primitive-service gate.** Narrow final-response Link capture, Link parser
   and page transition, root-array decoder, aggregate budget tracker, and their
   DuckDB-free exact/+1/counterexample tests pass independently. No production
   executor accepts pagination yet.
4. **Stream-contract and authorization gate.** Publish the nonempty-success
   comments/oracles, repeated scan-owned bearer decoration, complete profile
   validation, and I/O-free open. Preserve the two one-request profiles and
   prove wrong plans/capabilities fail before transport.
5. **Sequential-executor gate.** Integrate page state, budget tracker,
   transport, decode, and batching. The controlled executor oracle proves
   multi-page and empty-page traversal, no prefetch, canonical requests, all
   terminal failures, cancellation/close, one deadline, redaction, capability
   release, and recovery.
6. **Real-curl and boundary gate.** Prove terminal-block Link capture and
   repeated wire authorization through the private loopback service, preserve
   TLS/DNS/ambient-authority/header/decompression controls, and remove
   Connector ownership from `LoopbackCurlRuntime`. Installed-artifact canaries
   exclude every controlled authority and parser fixture hook.
7. **Controlled-product gate.** Lead and Query assemble Connector's catalog and
   Runtime's executor as separate providers through the unchanged production
   adapter path. Runtime tests remain independently executable without DuckDB;
   Query owns only SQL/lifecycle assertions and does not duplicate Link logic.
8. **Public-activation gate.** Only after the real Runtime accepts the exact
   repository plan does integration add the three-relation `0.5.0` catalog and
   identities. A controlled SQL narrative passes before any live credential
   use.
9. **Verification and exit gate.** Run focused parser, budget, decoder,
   executor, authorization, curl pagination, lifecycle, TLS, policy, and
   regression targets first; then the integrated `make build`, `make test`,
   `make demo`, source/dependency identity gates, and a fresh native product
   cell. Complete at least two fresh adversarial reviews over
   credential/authority/parser/resource and concurrency/cancellation/cleanup,
   resolve supported findings, stage the intended change for cached-diff
   checks, and audit final declarations, includes, factories, source groups,
   tests, documentation, and artifact inventories against the exits below.
...
- Beside Link parsing and page state, document the accepted grammar subset,
...
- **Relational Semantics — Open; Collaboration, then X-as-a-Service.** Exit
  when Runtime production includes only the public immutable plan contract,
...
- **Connector Experience — Open; Collaboration, then X-as-a-Service.** Exit
  when Runtime has no production include, construction, retained object, or
...
- **Query Experience — Open; Collaboration, then X-as-a-Service.** Exit when
  Query consumes only public authorization/executor/stream/batch/control/error
...
- **Remote Runtime workstream — Open.** Exit when DuckDB-free parser,
