...
Developer builds are explicitly non-release evidence. The fresh product
runner, tagged release gate, sanitizer cell, manifests, negative
...
checksummed tools, exact source checkouts, and the pinned Python host. Damaged
extracted tools are atomically restored from their verified cached archives.
The build service beside it owns tracked-source synchronization, incremental
Ninja state, test and isolated demo execution, paths, and fresh-runner
delegation. Its `paths` output
...
- required `test/python/source_demo_contract.py PINNED_PYTHON ARTIFACT`; and
...
contract. `make verify` allocates a new root and delegates to the
fresh product runner without reusing developer state.
...
reason; and independently runs the fresh and release evidence paths.
