...
void RequireRejected(const std::vector<std::string> &fields, LinkPaginationErrorKind expected_kind,
	...
	try {
		...
		Require(error.SafeMessage().find(CANARY) == std::string::npos,
		        label + " exposed received Link data");
...
void TestAcceptedTransitions() {
	LinkPaginationState state;
	const auto second = state.Advance(
...
void TestMalformedLinkGrammar() {
	const std::vector<std::pair<std::string, std::string>> cases = {
	    {"", "empty physical field"},
	    {"   \t", "OWS-only physical field"},
	    ...
	    {"<https://api.github.com/user/repos?per_page=100&page=2>; rel=next; REL=prev", "duplicate rel"},
	    {"<https://api.github.com/user/repos?per_page=100&page=2>; rel=next,", "trailing combined value"},
