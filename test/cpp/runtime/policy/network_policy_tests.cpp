...
bool AllowedIpv4(const char *text) {
	...
	return duckdb_api::internal::IsPublicSocketAddress(reinterpret_cast<const sockaddr *>(&address), sizeof(address));
}
...
void TestMalformedSocketMetadataDenied() {
	...
	address.sin_family = AF_INET;
	Require(
	    !duckdb_api::internal::IsPublicSocketAddress(reinterpret_cast<const sockaddr *>(&address), sizeof(address) - 1),
