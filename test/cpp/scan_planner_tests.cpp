...
void RequireColumn(const duckdb_api::PlannedColumn &column, const std::string &name,
                   const std::string &logical_type, const std::string &extractor) {
...
void TestTypedImmutableLivePlan() {
	Require(std::is_copy_constructible<duckdb_api::ScanPlan>::value, "ScanPlan must be copy-constructible for bind state");
	Require(std::is_move_constructible<duckdb_api::ScanPlan>::value, "ScanPlan must be move-constructible for bind state");
	...
	Require(plan.Domain() == duckdb_api::BaseDomain::SINGLE_RESPONSE_PAGE,
	        "ScanPlan widened the bounded base domain");
	...
	Require(operation.base_url == "https://api.github.com" && operation.path == "/search/users" &&
	...
	Require(operation.query_parameters.size() == 2 && operation.query_parameters[0].name == "q" &&
	            ...
	            operation.query_parameters[1].encoded_value == "3",
	...
	Require(operation.headers.size() == 3 && operation.headers[0].name == "Accept" &&
	            ...
	            operation.headers[1].name == "User-Agent" &&
	            operation.headers[1].value == "duckdb-api/0.3.0" &&
	...
	Require(plan.Pagination() == duckdb_api::FeatureState::DISABLED &&
	            ...
	            plan.Retry() == duckdb_api::FeatureState::DISABLED && plan.Cache() == duckdb_api::FeatureState::DISABLED &&
	...
	Require(plan.Budgets().IsLiveRestBudget(), "ScanPlan effective resource budget drifted");
	Require(plan.ClassificationReason() ==
...
void TestPrivateControlledCapabilityUsesTheSamePlanner() {
	auto connector = duckdb_api::BuildNativeGithubConnector();
	connector.operation.request.base_url = "http://127.0.0.1:8080";
	...
	const auto plan = duckdb_api::BuildConservativeScanPlan(connector, BuildLiveRequest(connector));
	Require(plan.Operation().base_url == "http://127.0.0.1:8080" && plan.Network().loopback_addresses_enabled,
...
void TestConnectorAndRequestCounterexamples() {
	...
	connector = duckdb_api::BuildNativeGithubConnector();
	connector.operation.request.base_url = "http://api.github.com";
	RequireConnectorRejected(connector, "outside declared scheme");
	connector = duckdb_api::BuildNativeGithubConnector();
	connector.operation.request.base_url = "https://example.com";
	RequireConnectorRejected(connector, "outside declared host");
	connector = duckdb_api::BuildNativeGithubConnector();
