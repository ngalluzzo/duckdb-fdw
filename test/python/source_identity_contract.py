...
CURRENT_PINS = json.loads((REPOSITORY / "release/0.10.0/pins.json").read_text())
...
SOURCE_PATHS = (
    ...
    "release/0.10.0/pins.json",
    "release/0.10.0/public_contract.json",
...
class SourceIdentityContractTests(unittest.TestCase):
    ...
    def test_rejects_source_path_pin_substitution(self) -> None:
        ...
        for label, paths in mutations.items():
            with self.subTest(label=label):
                pins = self.json("release/0.10.0/pins.json")
                pins["identities"]["native_product_sources"]["paths"] = paths
                self.write_json("release/0.10.0/pins.json", pins)
                with self.assertRaisesRegex(
                    AssertionError, "repository-relative|sorted unique"
                ):
                    VERIFIER.verify(self.root)
                self.restore("release/0.10.0/pins.json")
    ...
    def test_configured_build_graph_must_match_release_order_and_inventory(self) -> None:
        pins_path = self.root / "release/0.10.0/pins.json"
        expected = self.json("release/0.10.0/pins.json")["identities"][
            "build_graph"
        ]
...
""".lstrip()
  ...
  CONTROLLED "duckdb_api_controlled_loadable_extension")"""
    ...
    def test_rejects_current_contract_and_identity_pin_drift(self) -> None:
        contract = self.json("release/0.10.0/public_contract.json")
        contract["relations"][1]["cardinality"]["maximum"] = 2
        self.write_json("release/0.10.0/public_contract.json", contract)
        with self.assertRaisesRegex(AssertionError, "public contract digest"):
            VERIFIER.verify(self.root)
        self.restore("release/0.10.0/public_contract.json")
    ...
    def test_rejects_duplicate_json_keys(self) -> None:
        path = self.root / "release/0.10.0/pins.json"
...
"""
    ...
    def test_rejects_anything_except_the_single_extension_declaration(self) -> None:
        ...
        invalid = {
            "comment decoy": '# EXTENSION_VERSION "0.10.0"\n',
            "false block": (
                "if(FALSE)\n"
            ...
            "unquoted version": accepted.replace('"0.10.0"', "0.10.0"),
            "nonrelease version": accepted.replace('"0.10.0"', '"../../0.1.0"'),
        }
